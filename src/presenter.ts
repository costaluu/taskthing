...
import type { DownloadPhase } from "./download-progress";
...
export interface Presenter {
  ...
  /**
   * Like {@link Presenter.progressDynamic}, but the work reports which phase
   * it's in (downloading, with a byte count, or applying) for a live bar —
   * used by update apply. Piped output ignores the reports and just prints
   * the settled line.
   */
  downloadProgress(
    pending: string,
