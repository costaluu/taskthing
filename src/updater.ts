...
/** Fetches an asset's bytes from its URL, optionally reporting bytes as they arrive. */
...
export interface ApplyOptions {
  ...
  /** Bytes as they arrive during the binary download, for a progress bar. */
  onProgress?: (downloaded: number, total: number | null) => void;
  /** Fired once the binary is downloaded, before the swap + migrations run. */
  onApplying?: () => void;
}
...
export function createUpdater(deps: UpdaterDeps): Updater {
  ...
  async function apply(options: ApplyOptions): Promise<ApplyResult> {
    ...
    const state = await deps.config.read();
    if (state.autoUpdate === "confirm" && !(await options.confirm())) {
    ...
    const bytes = await deps.downloader.download(asset.url, options.onProgress);
    ...
    // Clear the pending flag now that it's installed, or the next check would
    // see the stale cache and report this same version as still pending.
    await deps.config.write({
      ...state,
      lastCheck: deps.clock.now(),
